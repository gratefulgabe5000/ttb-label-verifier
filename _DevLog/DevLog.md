...
### 2026-06-11 — Session 11: Synthetic Test Data — TS-01 Tier Sample Forms (2.2), "Good" Comparison Sets (2.3), "Hard Failure" Sets (2.4), "Possible Allowable Revision" Sets (2.5), Degraded Images (2.6) & Type 14b Set (2.7) — completes WBS 2.0
...
**Outcome (2.6):** WBS 2.6 complete — `testdata/degraded/*.jpg` (4 files) and `testdata/degraded_images.json` provide one degraded variant per FR-039 preprocessing concern (deskew/perspective correction, glare suppression, contrast normalization) plus one combined angle+glare fixture matching FR-039's literal acceptance test, all derived from a label image already woven through the 2.3-2.5 comparison fixtures. Approved by Gabe; proceeded to WBS 2.7.

**Completed (2.7):** WBS 2.7 builds a Type 14b ("for sale in one state only") application + matching label set, per FR-056 ("verify that at least one of the application's label images contains 'For sale in [STATE] only' when Form Item 14b is checked, using the state abbreviation declared in Item 14b... process 14b application with the correct statement on any one label image; confirm MATCH. With the statement on no image; confirm HARD_FAILURE.") This is the final item of WBS 2.0.
- **Scoping research before building:** FR-056 names two test cases — statement present (-> MATCH) and statement absent (-> HARD_FAILURE). The already-approved WBS 2.4 fixture `hf_14b_woodford` (Item 14b checked for Ohio, no "FOR SALE IN OHIO ONLY" statement on any of the 3 submitted Woodford Reserve label images, Type 14b row `HARD_FAILURE`, `expected_outcome: "DENY"`) already covers the second case in full. WBS 2.7's contribution is therefore the first case: a "good"-style 14b application where the matching statement IS present on a submitted label image, so the Type 14b row resolves to `MATCH` and `expected_outcome: "APPROVE"`. This avoids duplicating `hf_14b_woodford`'s coverage, following the same scoping-decision-documentation pattern used in 2.5 (the "color/font differences" scope-out).
- **Product selection:** real label images rarely carry a printed "For sale in [STATE] only" statement — it's added as a separate small statement label/sticker for products sold under a state-restricted Certificate of Exemption from Label Approval (27 CFR 13). No such image exists in `testdata/`, so a synthetic one was needed. Reviewed the unused (per 2.1-2.5) "Stoll & Wolfe" American Straight Whiskey label images (`Stoll & Wolfe whiskey front.jpg`/`back.jpg` — domestic, "Bottled by Heritage Spirits LLC - Lancaster, PA") and selected this product over reusing Woodford Reserve again, both for corpus diversity and because its real Lancaster, PA bottler address gives a natural, plausible state for a synthetic "FOR SALE IN PENNSYLVANIA ONLY" statement.
- Built `testdata/build_type14b_sets.py` on top of `formlib.py`, importing `_COMMON` from `build_good_sets.py`.
- **`testdata/synthetic/stollwolfe_for_sale_pa.jpg`** — a new Pillow-generated statement-label mockup (cream background, dark brown border, "CERTIFICATE OF EXEMPTION FROM LABEL APPROVAL" header + bold "FOR SALE IN PENNSYLVANIA ONLY" body text), saved to a new `testdata/synthetic/` directory to keep it clearly separated from the real photographed label images cataloged in `manifest.json`. **Sizing note:** the first render used a 600x180px canvas, but both lines of text measured 587-599px wide and overflowed past the drawn border (visually verified via the Read tool); widened the canvas to 700x180px and re-rendered — both lines now fit cleanly within the border.
- **`testdata/forms/type14b_match_stollwolfe.pdf`** (Stoll & Wolfe American Straight Whiskey blend, domestic) — built from `_COMMON` with Item 2 = "DSP-PA-19", Check Box34 = `/Domes` (Item 3 Domestic), Check Box22 = `/Spirits` (Item 5 Distilled Spirits), Item 6 = "Stoll & Wolfe", Item 7 blank, Item 8 = "Heritage Spirits LLC, 450 Distillery Lane, Lancaster, PA 17602", and Item 14 switched from 14a to **14b checked with state abbreviation "PA"** (`"14a. CERTIFICATE OF LABEL APPROVAL": "/Off"`, `"14b. CERTIFICATE OF EXEMPTION FROM LABEL APPROVAL": "/yes"`, `"14 b (Fill in State abbreviation)": "PA"`). All other fields match the real Stoll & Wolfe label images (front: "A Blend of American Straight Whiskeys", "43% ALC. by VOL. (86 PROOF)", "750ml"; back: full Government Warning, "Bottled by Heritage Spirits LLC - Lancaster, PA.").
- Verified `type14b_match_stollwolfe.pdf` via `pypdf` field readback (correct `/V` values incl. `14a=/Off`, `14b=/yes`, state=`PA`, multi-line Item 8 address) — note: the OUTPUT PDF from `fill_form()` is already decrypted, so reading it back must NOT call `.decrypt("")` (that raises `pypdf.errors.PdfReadError: Not encrypted file` on an already-unencrypted file, unlike the encrypted TEMPLATE) — and a 150dpi visual render of page 1 (Item 14b box checked, "PA" fits cleanly between the underscores in "'For sale in ___ only'", no overflow/clipping anywhere on the page).
- Updated `testdata/test_sets.json`: top-level `wbs_ref` -> `"2.3-2.5, 2.7"`, `source_note` extended to describe the 2.7 methodology (Item 14b checked + matching statement on a submitted image -> Type 14b row `MATCH`, `expected_outcome: "APPROVE"`; cross-references `hf_14b_woodford` for the HARD_FAILURE half; notes that the matching label image is a Pillow-generated mockup in `testdata/synthetic/` since no real photographed label carries this statement). Appended 1 new entry (`type14b_match_stollwolfe`, `wbs_ref: "2.7"`, application_type `"14b"`, expected_outcome `"APPROVE"`) with the same 11-row Sec. 2.5 Comparison Matrix structure as the 2.3/2.4/2.5 entries — Type 14b row `MATCH` (with a note cross-referencing `hf_14b_woodford`), Government Warning row using the full-text-with-trailing-period convention (matching `hf_14b_woodford` and `ar_brandname_fortemasso`, the convention used by the other already-approved distilled-spirits/14b entries).

**Outcome (2.7):** WBS 2.7 complete — `testdata/forms/type14b_match_stollwolfe.pdf`, `testdata/synthetic/stollwolfe_for_sale_pa.jpg`, and `testdata/test_sets.json` (now 18 total sets) provide the FR-056 MATCH-path comparison-engine fixture (Item 14b checked + matching "For sale in [STATE] only" statement present -> MATCH -> APPROVE), complementing the already-approved `hf_14b_woodford` (WBS 2.4) which covers FR-056's HARD_FAILURE path. **This completes WBS 2.0 (Synthetic Test Data Preparation) in full — all of 2.1-2.7 done.** Approved by Gabe; proceeded to WBS 3.0.

---

### 2026-06-11 — Session 12: Backend Auth — Agent Model & Seed (3.1), JWT Login (3.2), Current-Agent Dependency (3.3), Unit Tests (3.4) — completes WBS 3.0

**Context:** With WBS 2.0 complete and approved (Session 11), began WBS 3.0 — Backend Authentication & Authorization, traced to SR-001/SR-002 and Decision 8 #15 (JWT via python-jose + passlib). The `Agent` ORM model (`id`, `username` unique, `display_name`, `password_hash`, `created_at`) already existed from WBS 1.0 scaffolding. Before writing any backend code, read the already-built frontend (`web/src/contexts/AuthContext.tsx`, `web/src/lib/api-client.ts`, `web/src/lib/types.ts`, `web/src/pages/LoginPage.tsx` — WBS 11.6) to confirm the exact contract it expects: `POST /auth/login` with a JSON `{username, password}` body, returning `{access_token, token_type}`; the JWT is decoded client-side via `atob()` and must carry `sub` (username) and `display_name` claims; the token is stored in `localStorage` and sent as `Authorization: Bearer <token>` on subsequent requests via `apiFetch`.

**Completed (3.1 — Agent model + seed):**
- Built `app/services/auth_service.py`: `hash_password`/`verify_password` via `passlib.context.CryptContext(schemes=["bcrypt"])`; `authenticate_agent(db, username, password)` looks up the agent by username and verifies the password; `create_access_token`/`decode_access_token` via `python-jose`, using the existing `config.py` settings (`jwt_secret`, `jwt_algorithm="HS256"`, `jwt_expire_minutes=480`). The JWT payload carries `sub` (username), `agent_id`, `display_name`, and `exp` — `agent_id` is included now as the foundation for SR-002's per-agent data scoping in WBS 4.0+.
- **Dependency fix:** the project's existing `passlib[bcrypt]>=1.7.4` pin resolves to `bcrypt 5.0.0`, which raised `AttributeError: module 'bcrypt' has no attribute '__about__'` and then `ValueError: password cannot be longer than 72 bytes...` when `CryptContext(schemes=["bcrypt"]).hash(...)` was called — passlib 1.7.4 (released ~2020) predates bcrypt's removal of the `__about__` module attribute and its stricter 72-byte validation, both of which break passlib's internal backend-detection. Fixed by pinning `bcrypt==4.0.1` in `app/requirements.txt` (with an explanatory inline comment) and installing it into the venv. Verified with a direct hash/verify round-trip (`hash("test1234")` then `verify("test1234", h) == True`, `verify("wrong", h) == False`).
- Built `app/seed.py` — an idempotent script (safe to re-run; existing usernames skipped) that creates two demo agents: `agent1`/`agent2` (display names "Agent One"/"Agent Two"), both with password `password123`. No agent personas are established anywhere in PRD/DevLog/WBS, so generic names were chosen as placeholders. Documented the credentials in a new comment block in `app/.env.example` and added `python seed.py` to `README.md`'s "Backend Setup" steps (between `cp .env.example .env` and `uvicorn main:app --reload`) — this fulfills the README's pre-existing "First Run" step 2 forward-reference, "Log in as a test agent (credentials in `.env.example`)", which previously had no corresponding setup step. Ran `python seed.py` against the dev SQLite DB (`workingfiles.db`) — both agents created successfully.

**Completed (3.2 — `POST /auth/login`):**
- New `app/schemas/auth.py`: `LoginRequest {username: str, password: str}` and `TokenResponse {access_token: str, token_type: str = "bearer"}`, matching the frontend's `LoginRequest`/`LoginResponse` TypeScript interfaces field-for-field.
- New `app/routers/auth.py`: `POST /auth/login` calls `auth_service.authenticate_agent`; on success, issues a JWT via `create_access_token` and returns `TokenResponse`. On failure — either an unknown username or a wrong password — returns the same generic `401 {"detail": "Incorrect username or password."}` with a `WWW-Authenticate: Bearer` header, deliberately not distinguishing the two cases to avoid username enumeration. Wired into `main.py` (new router import + `app.include_router(auth.router)`).

**Completed (3.3 — current-agent dependency):**
- New `app/dependencies.py`: `get_current_agent`, using `HTTPBearer(auto_error=False)` rather than `OAuth2PasswordBearer` — `/auth/login` is JSON-bodied, not OAuth2-form-encoded, so `OAuth2PasswordBearer`'s built-in `/token`-style form handling doesn't fit. Decodes the bearer token via `auth_service.decode_access_token` (raises `jose.JWTError`, including `ExpiredSignatureError`, on any invalid/expired token) and looks up the `Agent` by the `sub` claim. Any failure mode — missing `Authorization` header, malformed/invalid/expired token, or a `sub` that no longer matches an `Agent` row — raises the same `401 {"detail": "Could not validate credentials."}` with `WWW-Authenticate: Bearer`, again without distinguishing failure modes in the response.
- Applied `dependencies=[Depends(get_current_agent)]` at the `APIRouter(...)` level to `app/routers/settings.py` — this is the only protectable router that exists at this point in the build (all of `/settings/api-key`'s GET/PUT/DELETE now require a valid bearer token). Future routers built in WBS 4.0+ will get the same router-level dependency as they're created.

**Completed (3.4 — unit tests):**
- New `app/tests/test_auth.py` (8 tests): login success returns `{access_token, token_type: "bearer"}`; login with the wrong password returns 401; login with an unknown username returns 401; the decoded access token's payload contains `sub`, `agent_id`, and `display_name` (the SR-002 data-scoping foundation); a protected route (`/settings/api-key`) without an `Authorization` header returns 401; with a valid token returns 200; with a garbage/invalid token returns 401; with an already-expired token returns 401.
- New `tests/conftest.py` fixtures: `test_agent` (inserts a known `testagent`/`testpass123` `Agent` row directly via `SessionLocal()` + `auth_service.hash_password`) and `auth_headers` (logs that agent in via `POST /auth/login` and returns `{"Authorization": "Bearer <token>"}`). The expired-token test constructs the token directly via `auth_service.create_access_token(test_agent, expires_delta=timedelta(minutes=-1))`, avoiding any real `sleep`.
- Updated `app/tests/test_settings.py`'s 3 existing tests (`test_api_key_not_configured_by_default`, `test_set_api_key_masks_value_and_does_not_persist`, `test_delete_api_key_clears_env`) to pass `headers=auth_headers`, since `/settings/api-key` is now behind `get_current_agent`.
- **Cross-thread `:memory:` SQLite fix (`app/db.py`):** the new `test_agent` fixture inserts a row via a `SessionLocal()` call made directly from the pytest test function, outside the request/response cycle. SQLAlchemy's default pooling for `sqlite:///:memory:` is `SingletonThreadPool` — one connection (and therefore one independent in-memory database) per thread. FastAPI's `TestClient` runs request handling via anyio's portal/threadpool, on a different thread than the pytest test function, so the `test_agent` row inserted from the test function would land in a *different*, empty `:memory:` database than the one `/auth/login`'s `get_db()`-injected session sees — the login would always fail with "unknown username." Fixed by adding `poolclass=StaticPool` to the `create_engine(...)` call specifically when `DATABASE_URL == "sqlite:///:memory:"` (the file-based SQLite branch, used in dev/prod, is unaffected) — `StaticPool` shares a single connection across the entire engine regardless of thread, which is the standard FastAPI-documented pattern for `TestClient` + in-memory SQLite.
- **Test isolation follow-on:** because `StaticPool` now keeps the in-memory database alive for the lifetime of the pytest process (rather than a fresh empty DB per `TestClient`), the `client` fixture's teardown now calls `Base.metadata.drop_all(bind=engine)` after each test, so a `testagent` row inserted by one test doesn't collide (`UNIQUE constraint failed: agents.username`) with the next test's `test_agent` fixture. `init_db()`'s `Base.metadata.create_all()` — already called via the app's `lifespan` on every new `TestClient` — recreates the schema from scratch for the next test.
- All 12 tests pass (`pytest -v`): the 8 new `test_auth.py` tests, `test_health.py`'s 1 test, and `test_settings.py`'s 3 updated tests.

**Outcome (3.0):** WBS 3.0 (Backend — Authentication & Authorization) complete. `POST /auth/login` issues JWTs matching the already-built frontend's expected contract exactly (`{access_token, token_type}`, with `sub`/`display_name`/`agent_id` claims); `get_current_agent` enforces SR-001 (login required for access) on `/settings/*`, the only protectable router that exists so far; demo credentials (`agent1`/`agent2`, both `password123`) are seeded via `app/seed.py` and documented in `.env.example`/`README.md`. This unblocks the frontend's existing `LoginPage`/`AuthContext`/`ProtectedRoute` (WBS 11.6) for end-to-end login testing against the real backend. SR-002 (per-agent data isolation) has its foundation in place via the `agent_id` JWT claim, to be enforced as application-data endpoints are built starting WBS 4.0. **Approved by Gabe** after end-to-end login testing against the running dev servers.

---

### 2026-06-11 — Session 13: Backend — Stage 1-2 Ingestion — Upload Endpoint (4.1), File Validation (4.2), Persistence (4.3), List/Filter (4.4), Detail (4.5), Unit Tests (4.6) — completes WBS 4.0

**Context:** With WBS 3.0 complete and approved (Session 12), began WBS 4.0 — Backend Stage 1-2 Ingestion, traced to DevLog §3.2 Stages 1-2, §3.5 (API surface), and PRD requirements FR-001–007, FR-070–072, IR-002, IR-003. Before writing code, reviewed the existing ORM models (`Application`, `LabelImage`, `FormParameter`, `LabelParameter`, `Determination`) and the pre-built frontend's `web/src/lib/types.ts` and `web/src/lib/api-client.ts` to confirm the exact request/response shapes `applicationsApi.upload()`/`.list()`/`.get()` expect, so the new endpoints could be wired up by the frontend without further API changes.

**Completed (4.1-4.3 — upload endpoint, validation, persistence):**
- New `app/schemas/application.py`: Pydantic response models mirroring the ORM — `LabelImageOut`, `FormParameterOut`, `LabelParameterOut`, `DeterminationOut`, `ApplicationOut` (includes the COLA Public Registry forward-compatibility fields per IA-22), and `ApplicationDetailOut(ApplicationOut)` adding `label_images`, `form_parameters`, `label_parameters`, `determination`.
- New `app/services/application_service.py`: `validate_form_file`/`validate_label_image` (FR-001/002/007, IR-002/003) check file content via magic-byte signatures (`%PDF`, `\xff\xd8\xff`, `\x89PNG\r\n\x1a\n`, `RIFF`...`WEBP`) rather than trusting `content_type`/extension, raising a `FileValidationError` with a plain-English message naming the offending file (FR-007). `create_application(...)` inserts the `Application` row first (commit+refresh to obtain `id`), then writes `form{ext}` and `label_{n}{ext}` under `UPLOAD_DIR/{application_id}/` and inserts the corresponding `LabelImage` rows (IA-26 storage layout).
- New `app/routers/applications.py`: `POST /applications/upload` (FR-001-006) creates one application per call; FR-006 "batch ingestion" is interpreted as repeated calls to this endpoint, matching `applicationsApi.upload(formData) → ApplicationDetail` (singular) in the frontend contract. Router-level `dependencies=[Depends(get_current_agent)]` (SR-001), following the pattern established for `settings` in WBS 3.0. New applications are created with `status = "PENDING"` — kept as-is rather than the DevLog prose's `PENDING_ASSESSMENT`, since `PENDING` is the value present in the frontend's `ApplicationStatus` enum (`PENDING | FORM_ASSESSED | LABEL_ASSESSED | COMPARED | COMPLETE`). Wired into `main.py`.

**Completed (4.4-4.5 — list/detail endpoints):**
- `GET /applications` (FR-070-072): scoped to `assigned_agent_id == current_agent.id` (SR-002), with an optional `applicant_name` substring filter via `ILIKE`, ordered newest-first, returning `list[ApplicationOut]`.
- `GET /applications/{id}` (DevLog §3.5): returns `ApplicationDetailOut` via a `_to_detail(db, application)` helper — `ApplicationOut.model_validate(application).model_dump()` spread together with a separately-queried `label_images` list (`application_service.list_label_images`) and empty placeholders for `form_parameters`/`label_parameters`/`determination` (populated by later WBS items). Returns `404` (not `403`) for another agent's application to avoid leaking existence (SR-002).

**Completed (4.6 — unit tests):**
- New `app/tests/test_applications.py` (11 tests): upload creates an application with label images and returns the full `ApplicationDetailOut` shape; a non-PDF form is rejected with a message naming the file and "PDF"; JPEG/PNG/WebP label images are all accepted; an invalid label image format is rejected with a message naming the file; 5 sequential uploads create 5 distinct applications (FR-006) and all appear in the list; `applicant_name` substring filtering; an agent's list excludes another agent's applications (SR-002); the detail endpoint returns the full shape including `label_images`; the detail endpoint returns 404 for another agent's application; the 20MB form / 10MB image limits (IR-002/003) accept exactly-at-limit and reject one byte over.
- New `app/tests/conftest.py` fixtures `second_agent`/`second_auth_headers`, mirroring `test_agent`/`auth_headers`, to exercise SR-002 cross-agent isolation.
- All 23 tests pass (`./.venv/Scripts/python.exe -m pytest -v`): the 11 new `test_applications.py` tests plus the existing 12 from WBS 3.0.

**Verification:** Restarted the dev backend after discovering an earlier `--reload` cycle had left an orphaned worker process serving stale routes (both the old and new uvicorn processes were `LISTENING` on port 8000 simultaneously); killed the orphaned PIDs and confirmed via `/openapi.json` that the live process now exposes `/applications/upload`, `/applications`, and `/applications/{application_id}`. Ran an end-to-end smoke test against the dev DB: logged in as `agent1`, uploaded a real PDF + JPEG with `applicant_name`/`serial_number`/`label_types`, confirmed the response had `id`, `status: "PENDING"`, and a populated `label_images` array, confirmed on-disk persistence at `data/uploads/{id}/form.pdf` and `data/uploads/{id}/label_1.jpg`, and confirmed `GET /applications?applicant_name=Smoke` returned the filtered result. Cleaned up the smoke-test `Application`/`LabelImage` rows and the `data/uploads/1/` directory afterward so the dev DB stays clean for review.

**Known discrepancy (non-blocking):** the pre-built frontend's `web/src/lib/api-client.ts` currently types `applicationsApi.list()` as returning `ApplicationDetail[]`, but `GET /applications` returns the lighter `ApplicationOut[]` per FR-070-072's 5-field list view (no `label_images`/`form_parameters`/`label_parameters`/`determination`). `DashboardPage` (WBS 12.0) should either narrow its usage to the 5 listed fields or call `GET /applications/{id}` for full detail. No frontend changes were made this session; flagged for WBS 12.0.

**Outcome (4.0):** WBS 4.0 (Backend — Stage 1-2: Ingestion) complete. `POST /applications/upload`, `GET /applications`, and `GET /applications/{id}` are live, enforce SR-001/SR-002, and match the pre-built frontend's `applicationsApi` contract closely enough that `DashboardPage` (WBS 12.0) can be wired up against these endpoints with only the minor list-shape note above. 23/23 backend tests passing. Pending Gabe's approval.
