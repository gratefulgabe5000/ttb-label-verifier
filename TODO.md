...
| Frontend (`web/`) | ✅ WBS 11.0–15.0 complete (build & lint passing, 30/30 Vitest). **12.0/13.0 re-sequencing (WBS.md v2.1 §4 Note 7) finished (Session 21, WBS.md v2.3)** — Agent Dashboard + Application Detail View fully wired to the backend. **Session 23 (2026-06-12)** restructured the Detail View into a Results Sidebar (13.0 polish, build/lint/tsc clean). **Session 24 (2026-06-12)** implemented WBS 14.0 — `BatchReportPage.tsx` (counts by outcome, common-failure-type display, CSV export, print/PDF export), wired live to `GET /batch/{id}/report`, plus a "View Report" link from the Dashboard's batch summary. **Session 25 (2026-06-12)** closed out WBS 15.0 — 15.1–15.4 confirmed already wired (audit, no changes); 15.5 added plain-English error states for failed comparison loads (`ResultsSidebar`/`ParameterResultsTable`) and failed batch-status polling (`DashboardPage`, with Retry/Dismiss) — see [`WBS.md` v2.6](_DevLog/WBS.md) |
